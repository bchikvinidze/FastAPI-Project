...
def test_wallet_create_over_limit(client: TestClient) -> None:
    ...
    assert response.json() == {
        "error": {"message": "wallet limit reached. Can't create any new wallets."}
    }
...
def test_wallet_persists(client: TestClient) -> None:
    ...
    assert response.status_code == 200
    assert response.json() == {"wallet_address": address, "bitcoins": ANY, "usd": ANY}
...
def test_transaction_send_from_not_own(client: TestClient) -> None:
    ...
    assert transaction_response.json() == {
        "error": {"message": "Can only transfer from own wallet addresses"}
    }
...
def test_transactions_get(client: TestClient) -> None:
    ...
    send_amount = 0.7
    transaction_response = client.post(
        f"/transactions/{wallet_from}/{wallet_to}/{send_amount}",
        headers={"x-api-key": api_key},
    )
    ...
    assert get_response.json() == {
        "transactions": []
    }  # jerjerobit ar maq daimplementirebuli
