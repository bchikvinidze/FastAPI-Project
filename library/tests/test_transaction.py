...
def test_transaction_own(client: TestClient) -> None:
    ...
    wallet_from_response = client.get(f"/wallets/{wallet_from}",
                                      headers={'x-api-key': api_key})
    bitcoin_from_final = wallet_from_response.json()['bitcoins']
    wallet_to_response = client.get(f"/wallets/{wallet_to}",
                                    headers={'x-api-key': api_key})
...
def test_transactions_get(client: TestClient) -> None:
    ...
    assert get_response.json() == {'transactions': [{"address_from": wallet_from,
                                                     "address_to": wallet_to,
                                                     "amount": send_amount}]}
