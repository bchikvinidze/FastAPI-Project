...
from constants import ADMIN_API_KEY, TRANSACTION_FEE


def test_statistics_no_fee(client: TestClient) -> None:
...
def test_statistics_nonzero_fee(client: TestClient) -> None:
    # create user and wallets
    response = client.post("/users")
    api_key = response.json()["user"]["key"]
    ...
    response = client.post("/users")
    api_key2 = response.json()["user"]["key"]

    # wallet
    response = client.post("/wallets", headers={"x-api-key": api_key2})
    resp_wallet = response.json()["usd_wallet"]
    wallet_to = resp_wallet["wallet_address"]
